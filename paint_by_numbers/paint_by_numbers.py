...
def clean_image(image_array: np.ndarray, filter_strength: int = 14, template_window_size: int = 7, search_window_size: int = 21) -> np.ndarray:
    """
...
def euclidean_distance(from_color: np.ndarray, to_color: np.ndarray) -> np.ndarray:
    ...
    """
    return np.sqrt(np.sum(np.power(from_color-to_color, 2), axis=2))
...
def draw_contours(image_array: np.ndarray, color_list: np.ndarray, min_sized_contour: int = 20, colored_lines: bool = False, add_label: bool = True, show_colored: bool = False) -> np.ndarray:
    """
    ...
    for ind, color in enumerate(color_list):
        ...
        for contour in cnts:
            ...
            if width_ctr > min_sized_contour and height_ctr > min_sized_contour and cv2.contourArea(contour, True) <- 100:
                if colored_lines:
...
def create_image_by_kmeans(image_file: str, number_of_colors: int, min_sized_contour: int = 20, filter_strength: int = 14, colored_lines=False, add_label=True, show_colored=False, output_file: Optional[str]=None):
    """
    ...
    contoured_image = draw_contours(new_color_image, kmeans.cluster_centers_, min_sized_contour, colored_lines, add_label, show_colored)
    save_image(output_file, contoured_image)
    return contoured_image


def create_image_by_predefined_colors(image_file: str, color_list: np.ndarray, min_sized_contour: int = 20, filter_strength: int = 14, colored_lines=False, add_label=True, show_colored=False, output_file: Optional[str]=None):
    """
